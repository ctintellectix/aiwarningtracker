...
public class AlertGenerationServiceTests
{
    ...
    public async Task GenerateAsync_creates_alerts_for_score_deterioration_and_capex_ocf_stress()
    {
        ...
        var result = await new AlertGenerationService(db).GenerateAsync();
    ...
    [Fact]
    public async Task GenerateAsync_creates_alert_for_material_rise_in_slowdown_transcript_terms()
    {
        await using var db = await CreateDbAsync();
        var previous = await AddQuarterAsync(db, 2026, 1);
        var current = await AddQuarterAsync(db, 2026, 2);
        ...
        await new AlertGenerationService(db).GenerateAsync();

        Assert.Contains(db.WatchlistAlerts, x => x.Title.Contains("Negative transcript terms rose", StringComparison.OrdinalIgnoreCase));
    }
    ...
    public async Task GenerateAsync_creates_category_weakening_alerts_without_duplicates()
    {
        ...
        await db.SaveChangesAsync();
        var service = new AlertGenerationService(db);
